        ...
        {#if ubsImageResources.length}
            ...
            <div class="carousel w-full space-x-2 pb-6">
                {#each ubsImageResources as image}
                    <button class="carousel-item w-32 flex-col" on:click={() => handleMediaResourceSelected(image)}>
                        <img
                            class="mb-1 h-20 w-32 rounded-lg object-cover"
                            src={cachedOrRealUrl(image.url)}
                            alt={image.displayName}
                        />
                        <span class="line-clamp-1 break-all text-sm text-neutral">{image.displayName}</span>
                    </button>
                {/each}
            </div>
        {/if}
        {#if videoBibleDictionaryResources.length}
            ...
            <div class="carousel w-full space-x-2 pb-6">
                {#each videoBibleDictionaryResources as video}
                    <button class="carousel-item w-32 flex-col" on:click={() => handleMediaResourceSelected(video)}>
                        ...
                        <span class="line-clamp-1 break-all text-sm text-neutral">{video.displayName}</span>
                    </button>
                {/each}
            </div>
        {/if}
